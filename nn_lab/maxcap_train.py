...
def main():
    ...
    clf = LGBMClassifier(n_estimators=800, learning_rate=0.03, max_depth=10,
                         num_leaves=255, subsample=0.9, colsample_bytree=0.9,
    ...
    rg = LGBMRegressor(n_estimators=800, learning_rate=0.03, max_depth=10,
                       num_leaves=255, subsample=0.9, colsample_bytree=0.9,
                       min_child_samples=5, verbose=-1, n_jobs=4)
    sel_tr = tr & (ynet > 0)
    ...
    report = {"span_years": round(span_y, 2), "top_n": TOP_N,
              "train_precision_at_0.30": float(ybin[tr_sel].mean()) if tr_sel.sum() else 0.0,
              "train_n_at_0.30": int(tr_sel.sum()),
              "train_precision_label": "in-sample memorization — NOT live performance",
